...
class FractionDivision(Scene):
    def construct(self):
        ...
        self.play(
            lower_fraction.animate.shift(DOWN * 1),
            FadeOut(equation)
        )
        self.wait(0.5)

        # Invert the lower fraction (4/6 becomes 6/4)
        inverted_fraction = MathTex(r"\frac{6}{4}", font_size=34)
        inverted_fraction.set_color(RED)
        inverted_fraction.move_to(lower_fraction.get_center())

        # Show the inversion
        inversion_text = Text("Invert", font_size=24).next_to(
            lower_fraction, RIGHT)
        self.play(Write(inversion_text))
        self.wait(0.5)

        self.play(
            ReplacementTransform(lower_fraction, inverted_fraction),
            FadeOut(inversion_text),
            inverted_fraction.animate.scale(2),
            upper_fraction.animate.scale(2)
        )
        self.wait(0.5)
        self.play(
            upper_fraction.animate.shift(LEFT * 0.5),
            inverted_fraction.animate.shift(UP * 1.7 + RIGHT * 0.2),
        )
        self.wait(0.5)
        # Create the intermediate step
        intermediate = MathTex(r"\frac{5}{3} \times \frac{6}{4} = x")
        intermediate[0][0].set_color(BLUE)  # 5
        intermediate[0][2].set_color(BLUE)  # 3
        intermediate[0][4].set_color(RED)   # 6
        intermediate[0][6].set_color(RED)   # 4

        # Group the elements
        group = VGroup(upper_fraction, inverted_fraction)

        # Transform to the intermediate step
        self.play(
            ReplacementTransform(group, intermediate, run_time=0.2),
        )
        self.wait(1)

        # Continue with multiplication steps similar to FractionMultiplication
        # Create copies of the parts to be multiplied
        numerator_part = intermediate[0][:3].copy()  # "5 × 6"
        denominator_part = intermediate[0][4:7].copy()  # "3 × 4"

        # Move the copies to positions above and below the fraction
        self.play(
            numerator_part.animate.move_to(intermediate.get_center() + UP * 2),
